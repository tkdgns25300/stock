...
const FinancialRatio: React.FC<FinancialRatioProps> = ({ financialRatioData }) => {
	...
	const sixFinancialRatioData = financialRatioData.slice(-6);
	const fiveFinancialRatioData = financialRatioData.slice(-5);
				...
				<div className="bg-white p-5 rounded-xl shadow-xl font-doHyeon">
					<div className="flex justify-between">
						<div className="flex items-center">
							<div className="w-3 h-3 bg-[#5882FA] rounded-full mr-2"></div> 매출액 증가율 :
						</div>
						<div className="mx-2">{props.payload[0].value}</div>
					</div>
					<div className="flex justify-between">
						<div className="flex items-center">
							<div className="w-3 h-3 bg-[#82ca9d] rounded-full mr-2"></div> 영업이익 증가율 :
						</div>
						<div className="mx-2">{props.payload[1].value}</div>
					</div>
					<div className="flex justify-between">
						<div className="flex items-center">
							<div className="w-3 h-3 bg-[#FACC2E] rounded-full mr-2"></div> 순이익 증가율 :
						</div>
		...
		<div className="w-full">
			<div className="w-full mx-4 my-8">
				<ResponsiveContainer width="100%" height={400}>
					<BarChart data={fiveFinancialRatioData} margin={{ top: 20, right: 30, left: 20, bottom: 50 }}>
						...
						/>
						<Bar dataKey="grs" fill="#5882FA" name="매출액 증가율" barSize={25} radius={[5, 5, 5, 5]} />
						<Bar dataKey="bsopPrfiInrt" fill="#82ca9d" name="영업이익 증가율" barSize={25} radius={[5, 5, 5, 5]} />
						<Bar dataKey="ntinInrt" fill="#FACC2E" name="순이익 증가율" barSize={25} radius={[5, 5, 5, 5]} />
					</BarChart>
				</ResponsiveContainer>
			</div>
			{fiveFinancialRatioData.length > 0 && (
				<FinancialRatioQuartly
					curFinancialRatioData={fiveFinancialRatioData[selectedIndex]}
					previousFinancialRatioData={sixFinancialRatioData[selectedIndex]}
				/>
			)}
		</div>
	);
